...
def generate_notes_for_keyframes(keyframes, video_transcript):
    ...
    p = Pool(10)
    interval_notes = p.map(get_notes, [(n,interval) for n, interval in enumerate(intervals)])
...
def generate_pdf_from_interval_notes(interval_notes, outp):
    ...
    Story.append(Story.append(Paragraph(f"(Generated by KnosisAI)", subtitle_style)))
    ...
    for interval_num, interval_note in interval_notes:
        ...
        interval_note_stuff, mode = detect_note_len(interval_note)
        if mode == "h":
            nexto = interval_note_stuff[0]
            table_content = [interval_note, keyframe_im]
            if interval_num%2==1:
                table_content.reverse()

            Story.append(Table([table_content],style=chart_style))
            if len(interval_note_stuff)>1:
                Story.append(Paragraph(interval_note_stuff[-1],style=text_style))
        else: ##mode == "v"
            keyframe_im_large = get_image_preserve_ratio(f"imgtmps/image_{interval_num}.png",width=round(IMAGE_SIZE_LARGE*inch))
            Story.append(keyframe_im_large)
            print(interval_note_stuff[-1])
            Story.append(Table([[interval_note_stuff[-1]]],style=chart_style))
...
def create_notes_pdf(video_id):
    ...
    interval_notes = generate_notes_for_keyframes(video_keyframes,transcript)
    generate_pdf_from_interval_notes(interval_notes,"static/Notes.pdf")
